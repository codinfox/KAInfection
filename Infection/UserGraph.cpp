...
void UserGraph::importFromORM(const vector<User> &graph) {
...
void UserGraph::clustering() {
    ...
    for (User user : users) {
        ...
        size_t clusterID = clusters.add(unordered_set<user_id>());
        unordered_set<user_id> & cluster = clusters[clusterID];
        queue<user_id> userQ;

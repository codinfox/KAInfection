...
namespace KA {
    ...
    struct User {
        User(user_id _identifier, std::vector<user_id> const & _connections)
        : identifier(_identifier), connections(_connections), cluster(0) {}
        
        user_id identifier;
        std::vector<user_id> connections;
        size_t cluster;
    };
    ...
    class UserGraph {
        ...
        KVector<std::unordered_set<user_id>> clusters;
        KVector<ClusterMapping> aisles[total_aisles];
        
        bool initialized = false;
        
        void importFromORM(std::vector<User> const & graph);
        void clustering();
    ...
    public:
         ...
         */
        UserGraph(std::vector<User> const & graph);
         ...
         */
        std::unordered_set<user_id> findClusterByUser(user_id userID);
        /**
         ...
         */
        std::unordered_set<user_id> findClusterByAmount(size_t amount);
    };
}
